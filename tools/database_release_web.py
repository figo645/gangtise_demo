...
def _target(name):
    prefix = "STAGING" if name == "staging" else "PRODUCTION"
    default_host = "129.211.65.53" if name == "staging" else "47.105.48.193"
    ...
    return {
        ...
        "db_name": os.environ.get(
            f"DATABASE_RELEASE_{prefix}_DB_NAME", "sprint_dashboard"
        ),
        "db_user": os.environ.get(
            f"DATABASE_RELEASE_{prefix}_DB_USER", "postgres"
        ),
        "db_host": db_host,
        ...
        "db_password": os.environ.get(
            f"DATABASE_RELEASE_{prefix}_DB_PASSWORD", "your_password"
        ),
    }
...
poll();loadRollbacks();</script></body></html>
"""
...
def index():
    ...
    return render_template_string(HTML, configured=configured, packages=release_packages())

...
class ReviewModuleBddTest(unittest.TestCase):
    ...
    def test_given_workbench_when_review_file_or_url_is_processed_then_editor_handoff_logic_exists(self):
        ...
        self.assertIn("kwReviewDraft.urlText = extractedText;", html)
        self.assertIn("kwReviewDraft.sourceMode = 'url';", html)
        self.assertIn("id=\"kw-review-url-text\"", html)
        self.assertIn("网页内容会作为原始素材提交给大模型，生成结果进入 Draft 审核，不会直接发布。", html)
    ...
    def test_given_h5_review_when_page_renders_then_published_articles_use_current_tenant_pages(self):
        ...
        self.assertIn("const hasReviewRoute = ['compose', 'drafts'].includes(initialParams.get('review'))", html)
        ...
        self.assertIn("function getAllDavUsersForReviews()", html)
        self.assertIn("const dashboardReviews = Array.isArray(dashboard.reviews) ? dashboard.reviews : null;", html)
        self.assertIn("tenant.review_snapshots = dashboardReviews;", html)
        self.assertIn("rerenderReviews: true", html)

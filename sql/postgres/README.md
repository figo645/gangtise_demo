...
- Local web controller: [`tools/database_release_web.py`](/Users/xuchenfei/PycharmProjects/gangtise_demo/tools/database_release_web.py), started by [`start_database_release_web.sh`](/Users/xuchenfei/PycharmProjects/gangtise_demo/start_database_release_web.sh). Copy [`.database_release.env.example`](/Users/xuchenfei/PycharmProjects/gangtise_demo/.database_release.env.example) to `.database_release.env`, configure the staging and production targets, then open `http://127.0.0.1:5051/` to select the target and monitor one release job. The controller is local-only by default and permits only one concurrent database replacement.
- Direct start: `python3 tools/database_release_web.py`. Staging defaults to direct PostgreSQL connection `129.211.65.53:5432`; production defaults to `47.105.48.193:5432`. Both use database `sprint_dashboard` and user `postgres` unless overridden. No SSH is used by the database release flow.

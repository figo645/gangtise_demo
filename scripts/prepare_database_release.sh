...
pg_isready -h "$LOCAL_HOST" -p "$LOCAL_PORT" >/dev/null || { echo "Local PostgreSQL is unavailable." >&2; exit 1; }
pg_isready -h "$REMOTE_DB_HOST" -p "$REMOTE_DB_PORT" >/dev/null || { echo "${TARGET} PostgreSQL is unavailable at ${REMOTE_DB_HOST}:${REMOTE_DB_PORT}." >&2; exit 1; }

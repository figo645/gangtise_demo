...
DECLARED_AGENT_WORKFLOW_CATALOG = {
    "hermes_agent": {
        ...
        "tags": ["H5", "知识检索", "证据链", "自选股", "LLM"],
        "nodes": [
            {"id": "question_input", "label": "问题输入", "processor": "input", "kind": "source", "x": 36, "y": 88, "description": "接收用户问题、附件、知识范围和上下文。"},
            {"id": "intent_router", "label": "意图路由", "processor": "router", "kind": "llm_or_rule", "x": 320, "y": 88, "description": "优先用 LLM 识别意图，失败时退回规则路由。"},
            {"id": "tool_dispatch", "label": "工具调度", "processor": "tool_dispatch", "kind": "tooling", "x": 620, "y": 88, "description": "按路由计划调用知识检索、证据链、附件解析和个股详情。"},
            {"id": "answer_synthesis", "label": "答案合成", "processor": "llm_synthesis", "kind": "llm", "x": 920, "y": 88, "description": "根据工具结果做最终回答整合。"},
            {"id": "artifact_render", "label": "结果渲染", "processor": "artifact", "kind": "output", "x": 1212, "y": 88, "description": "生成结构化卡片或纯文本回答，并附带思考流与工具轨迹。"},
        ],
        "edges": [
            {"id": "edge_hermes_1", "from": "question_input", "to": "intent_router"},
            {"id": "edge_hermes_2", "from": "intent_router", "to": "tool_dispatch"},
            {"id": "edge_hermes_3", "from": "tool_dispatch", "to": "answer_synthesis"},
            {"id": "edge_hermes_4", "from": "answer_synthesis", "to": "artifact_render"},
        ],
    },

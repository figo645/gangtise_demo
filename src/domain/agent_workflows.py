...
DECLARED_AGENT_WORKFLOW_CATALOG = {
    "hermes_agent": {
        ...
        "nodes": [
            ...
            {"id": "scope_guard", "label": "安全边界检查", "processor": "scope_guard", "kind": "guardrail", "x": 650, "y": 464, "description": "阻断直接买卖、仓位和喊单请求；普通问题继续交给 LLM。意图路由不使用 embedding。"},
            {"id": "intent_router", "label": "LLM 意图拆解", "processor": "router", "kind": "llm", "x": 650, "y": 608, "description": "使用 Admin 当前默认模型，把一句话拆成一个或多个任务，输出 execute / clarify / chat / refuse。"},
            {"id": "refuse_branch", "label": "安全收口", "processor": "branch", "kind": "decision", "visual_only": True, "x": 30, "y": 800, "description": "条件：disposition = refuse。直接说明安全边界，不调用研究 API。"},
        ...
        "edges": [
            ...
            {"id": "edge_hermes_refuse", "from": "intent_router", "to": "refuse_branch", "label": "拒绝", "condition": "disposition = refuse"},
            {"id": "edge_hermes_clarify", "from": "intent_router", "to": "clarify_branch", "label": "需补充", "condition": "disposition = clarify"},
            {"id": "edge_hermes_chat", "from": "intent_router", "to": "chat_branch", "label": "闲聊", "condition": "disposition = chat"},
            {"id": "edge_hermes_single", "from": "intent_router", "to": "single_branch", "label": "单任务", "condition": "execute + 1 task"},
            {"id": "edge_hermes_composite", "from": "intent_router", "to": "composite_branch", "label": "多任务", "condition": "execute + 2+ tasks"},

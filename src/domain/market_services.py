...
def list_user_watchlist_items(tenant_slug="", user_profile_id="", allow_provider_fetch=False):
...
def call_gangtise_stock_one_pager(security_code, timeout=90):
    """Fetch the latest non-intraday Gangtise company one-pager for one stock."""
    normalized_code = str(security_code or "").strip().upper()
    if not normalized_code:
        raise ValueError("gangtise_one_pager_security_code_required")
    # The one-pager endpoint is singular: unlike stock-summary, it requires
    # one securityCode field rather than a securityList array.
    payload = {"securityCode": normalized_code}
    ...
    if not is_gangtise_openapi_success(status, response):
        message = str((response or {}).get("msg") or (response or {}).get("message") or "empty_response").strip()
        raise RuntimeError(f"gangtise_one_pager_failed:http_status={status or 0}:{message[:400]}")
    data = copy.deepcopy((response or {}).get("data"))
    report_date = str(data.get("date") or "").strip() if isinstance(data, dict) else ""
    ...
    return {
        ...
        "response_code": str((response or {}).get("code") or "").strip(),
        "response_status": (response or {}).get("status"),
        "duration_ms": int(duration or 0),
        "provider": "Gangtise 公司一页通",
        "endpoint": "/application/open-ai/agent/one-pager",
        "as_of_note": "报告为 Gangtise 最近一期结构化研究，非当日研究。",
    }


def call_gangtise_stock_summaries(security_codes, timeout=60):
    """Fetch concise per-security highlights; the upstream supports up to 6000 symbols."""
    normalized_codes = []
    for value in security_codes if isinstance(security_codes, list) else []:
        normalized = str(value or "").strip().upper()
        if normalized and normalized not in normalized_codes:
            normalized_codes.append(normalized)
    if not normalized_codes:
        raise ValueError("gangtise_stock_summary_security_list_required")
    if len(normalized_codes) > 6000:
        raise ValueError("gangtise_stock_summary_security_limit_exceeded")
    payload = {"securityList": normalized_codes}
    status, response, duration = post_gangtise_openapi_json(
    ...
    if not is_gangtise_openapi_success(status, response):
        message = str((response or {}).get("msg") or (response or {}).get("message") or "empty_response").strip()
        raise RuntimeError(f"gangtise_stock_summary_failed:http_status={status or 0}:{message[:400]}")
    ...
    return {
        ...
        "response": copy.deepcopy(response or {}),
        "duration_ms": int(duration or 0),
        "provider": "Gangtise 个股看点摘要",
        "endpoint": "/application/open-ai/stock-summary/getList",
        "limitation": "仅提供单只证券的精炼看点，不能替代完整观察报告。",
    }
...
def fetch_gangtise_indicator_series(indicator_code, start_date="", end_date="", token=""):
    ...
    # Market Overview and Hot Industries have one controlled provider: the
    # persisted AKShare snapshot. Never let a stale source definition or a
    # manual endpoint request fall back to a chargeable Gangtise API.
    if normalized_code in MARKET_OVERVIEW_INDEX_CODES or normalized_code == "source_industry_index":
        return {
            "ok": False,
            "message": "market_indicator_akshare_only",
            "points": [],
            "response": {},
            "duration_ms": 0,
    ...
    if query_kind == "index_kline":
        ...
        status, response, duration = post_gangtise_openapi_json(
            GANGTISE_INDEX_KLINE_DAILY_PATH,
            payload,
            token=token,
            timeout=30,
        )
        ...
        return {
            ...
            "response": response if isinstance(response, dict) else {},
            "duration_ms": int(duration or 0),
            "source_meta": {"type": "index_kline", "path": GANGTISE_INDEX_KLINE_DAILY_PATH, "securityCode": entry["security_code"]},
        }
...
def build_gangtise_source_seed_payload(indicator_code, existing=None):
    ...
    path = GANGTISE_INDEX_KLINE_DAILY_PATH if query_kind == "index_kline" else "/application/open-alternative/EDB/getData"
...
def sync_real_indicator_history_from_market_cache(force=False):
    ...
    enabled_sources = {
        ...
        for item in list_indicator_source_defs()
        if item.get("enabled") and infer_source_connector_type(item) == "gangtise_openapi"
    }
    sync_targets = [
        definition
        for definition in definitions
        if definition.get("enabled")
        and definition["indicator_code"] in GANGTISE_INDICATOR_REGISTRY
        and definition["indicator_code"] in enabled_sources
    ]
    skipped_disabled = sum(
        1
        for definition in definitions
        if definition["indicator_code"] in GANGTISE_INDICATOR_REGISTRY
        and (not definition.get("enabled") or definition["indicator_code"] not in enabled_sources)
    )
    # Admin source toggles are a cost-control boundary: do not even
    # authenticate when there is no eligible Gangtise source to synchronize.
    if not sync_targets:
        return {
            "synced": True,
            "updated": 0,
            ...
            "detail": "没有启用的 Gangtise 指标源，未发起外部请求。",
        }
    ...
    updated = 0
    total_points = 0
    for definition in sync_targets:
        ...
        if len(rows) < 2:
            continue
        source_code = enabled_sources[indicator_code]["source_code"]
    ...
    return {
        ...
        "eligible": len(sync_targets),
        "skipped_disabled": skipped_disabled,
        "batch_code": batch_code if updated else "",
    }
...
GANGTISE_MARKET_OVERVIEW_SPECS = (
    ...
    ("source_nikkei", "日经225", ("日经225指数", "日经225"), ("日经", "225")),
)
...
def fetch_gangtise_market_index_history(indicator_code, start_date, end_date):
    ...
    if entry.get("query_kind") == "index_kline":
        result = fetch_gangtise_market_kline_series(
            GANGTISE_INDEX_KLINE_DAILY_PATH, entry.get("security_code"),
        ...
        return {**result, "provider": "Gangtise OpenAPI", "source_meta": result.get("source_meta") or {"type": "index_kline", "path": GANGTISE_INDEX_KLINE_DAILY_PATH, "securityCode": entry.get("security_code")}}
...
def _fetch_gangtise_sector_overview(start_date, end_date, sector_names=None):
    ...
    for sector in selected_sectors:
        ...
        series = fetch_gangtise_market_kline_series(
            GANGTISE_INDEX_KLINE_DAILY_PATH, security_code,
            start_date=start_date, end_date=end_date, limit=240, timeout=30,
        )
        ...
        items.append({
            ...
            "source_meta": series.get("source_meta") or {"type": "index_kline", "path": GANGTISE_INDEX_KLINE_DAILY_PATH, "securityCode": security_code},
        })
...
def _preserve_last_market_snapshot_for_closed_session(payload, label):
    """Keep the last successful quote visible whenever China is not trading.

    Snapshot TTL protects a live session from silently showing an old quote. It
    must not blank the boards overnight, on weekends, or on exchange holidays:
    there cannot be a newer domestic quote in those periods. The H5 surface
    marks this payload as historical and retains its exact sync time.
    """
    result = copy.deepcopy(payload) if isinstance(payload, dict) else {"ok": True, "items": []}
    result["stale"] = True
    result["last_success_at"] = str(result.get("updated_at") or "")
    if not is_cn_stock_market_open():
...
def _apply_tenant_market_display_selection(payload, tenant_slug, kind):
    ...
    settings = load_tenant_market_display_settings(tenant_slug)
    selected = settings["market_overview_codes"] if kind == "market" else settings["sector_names"]
    item_key = "indicator_code" if kind == "market" else "sector"
    if settings.get("configured"):
        by_key = {str(item.get(item_key) or "").strip(): item for item in items if isinstance(item, dict)}
        result["items"] = [by_key[key] for key in selected if key in by_key]
    else:
        # A newly created tenant gets useful live data before its DaV makes a
...
def build_market_overview_payload(tenant_slug=""):
...
# /application/open-quote/index/kline/daily. They are index quote codes, not
# EDB IDs.
...
def _has_complete_shenwan_level1_sector_snapshot(items):
    """Require one valid quote for each official Shenwan level-one industry."""
    if not isinstance(items, list) or len(items) != len(SHENWAN_LEVEL1_INDUSTRIES):
        return False
    expected = set(SHENWAN_LEVEL1_INDUSTRIES)
    actual = {str((item or {}).get("sector") or "").strip() for item in items if isinstance(item, dict)}
...
def _fetch_akshare_sector_overview(ak=None):
    ...
    try:
        ...
        # Pandas Index deliberately has no boolean value. Materialize it once
        # so diagnostics cannot turn an otherwise valid provider response into
        # a failed industry snapshot.
        ...
        if not name_column or not code_column or not value_column or not previous_column:
            app.logger.warning(
                "AKShare Shenwan level-one response missing required columns columns=%s",
                columns,
            )
            return []
        items = []
        ...
        found_sectors = {item["sector"] for item in items}
        missing_sectors = [sector for sector in SHENWAN_LEVEL1_INDUSTRIES if sector not in found_sectors]
        if not _has_complete_shenwan_level1_sector_snapshot(items):
            # The industry board is a like-for-like ranking. Partial vendor
            # responses must not replace the last complete shared snapshot.
            app.logger.warning(
                "AKShare Shenwan level-one response incomplete rows=%s expected=%s missing=%s",
                len(items),
                len(SHENWAN_LEVEL1_INDUSTRIES),
                ",".join(missing_sectors),
            )
            return []
        items.sort(key=lambda item: (-float(item["change_pct"]), str(item["sector"])))
        app.logger.info(
            "AKShare Shenwan level-one snapshot accepted rows=%s columns=%s",
            len(items),
            columns,
        )
        return items
...
def build_market_sector_overview_payload(force_refresh=False, tenant_slug=""):
...
def resolve_watchlist_candidate(stock_code="", stock_name=""):
    """Resolve a stock to the canonical security record for cross-domain tools."""
    return _resolve_watchlist_candidate(stock_code=stock_code, stock_name=stock_name)
...
def _load_cached_watchlist_detail(candidate, include_stale=False):
    """Read a persisted quote only; this helper must never contact a provider."""
    normalized = candidate if isinstance(candidate, dict) else {}
    security_code = str(normalized.get("security_code") or "").strip().upper()
    if not security_code:
        return None
    ttl = 0 if include_stale else WATCHLIST_DETAIL_CACHE_TTL_SECONDS
    cached = _load_watchlist_cache("watchlist_detail_cache", security_code, ttl)
    if not _watchlist_detail_cache_is_usable(cached):
        return None
    detail = copy.deepcopy(cached)
    if include_stale and _load_watchlist_cache("watchlist_detail_cache", security_code, WATCHLIST_DETAIL_CACHE_TTL_SECONDS) is None:
        detail["stale"] = True
        detail["data_stale_message"] = "行情缓存超过 30 分钟，等待后台行情任务刷新。"
    return detail


def _build_watchlist_realtime_detail_from_candidate(candidate, stock_name="", allow_provider_fetch=True):
    normalized = candidate if isinstance(candidate, dict) else {}
    security_code = str(normalized.get("security_code") or "").strip().upper()
    if not security_code:
    ...
    cached = _load_cached_watchlist_detail(normalized)
    if cached:
        app.logger.warning(
        ...
        return attach_watchlist_intraday(cached, allow_provider_fetch=allow_provider_fetch)
    if not allow_provider_fetch:
        return _load_cached_watchlist_detail(normalized, include_stale=True)
    ...
    with fetch_lock:
        ...
        if cached:
            ...
            return attach_watchlist_intraday(cached, allow_provider_fetch=True)
        return _fetch_watchlist_realtime_detail_from_candidate(candidate, stock_name=stock_name)
...
def _merge_watchlist_intraday_candle(detail, intraday_points):
    """Expose the live minute quote as today's provisional daily candle.

    Sina's daily endpoint commonly publishes only the last completed session
    during market hours. The detail page already retrieves a real minute series,
    so use it to append/replace only today's candle without altering closed-day
    daily history.
    """
    if not is_cn_stock_market_open() or not isinstance(detail, dict):
        return detail
    trade_date = _current_cn_market_date().isoformat()
    points = [
        item for item in (intraday_points or [])
        if isinstance(item, dict)
        and str(item.get("date") or "")[:10] == trade_date
        and numeric_value(item.get("value")) is not None
    ]
    if not points:
        return detail
    points.sort(key=lambda item: str(item.get("date") or ""))
    values = [numeric_value(item.get("value")) for item in points]
    ...
    def merge_candles(candles):
        rows = [item for item in (candles or []) if isinstance(item, dict) and str(item.get("date") or "")[:10] != trade_date]
        rows.append(candle)
    ...
    detail["history_series"] = [
        {
            ...
            "value": item["close"],
            "status": build_real_indicator_status(
                NumberLike(item["close"]),
                NumberLike(history_candles[index - 1]["close"]) if index else NumberLike(item["close"]),
            ),
        }
        for index, item in enumerate(history_candles)
    ]
    previous = next((item for item in reversed(daily_candles[:-1]) if NumberLike(item.get("close")) > 0), None)
    if previous:
        previous_close = NumberLike(previous.get("close"))
        change = candle["close"] - previous_close
        detail["price"] = candle["close"]
        detail["change"] = round(change, 2)
        detail["change_pct"] = round(change / previous_close * 100, 2) if previous_close else 0.0
    detail["kline_contains_intraday"] = True
    detail["kline_intraday_updated_at"] = str(points[-1].get("date") or "")
    return detail


def attach_watchlist_intraday(detail, allow_provider_fetch=True):
    if not isinstance(detail, dict) or not detail:
        return detail
    ...
    # Page composition is cache-only. Provider access is reserved for a
    # focused detail request or a scheduler collection run.
    ...
    if detail["intraday_available"]:
        _merge_watchlist_intraday_candle(detail, detail["intraday_series"])
    return detail
...
def get_watchlist_detail_by_code(stock_code="", stock_name="", details_map=None, enrich=True, allow_provider_fetch=False):
    details = details_map if isinstance(details_map, dict) else gen_watchlist_details(allow_provider_fetch=allow_provider_fetch)
    raw_code = str(stock_code or "").strip()
    indicator_code = normalize_watchlist_indicator_code(raw_code)
    normalized_code = indicator_code or raw_code.upper()
    if normalized_code in GANGTISE_INDICATOR_REGISTRY:
    ...
    if not normalized_code:
        return None
    if not allow_provider_fetch:
        cached = _load_cached_watchlist_detail(
            _resolve_watchlist_candidate(stock_code=normalized_code, stock_name=stock_name),
            include_stale=True,
        )
        if cached:
            return copy.deepcopy((_enrich_watchlist_details({normalized_code: cached}).get(normalized_code)) or cached)
    ...
    if detail:
        ...
        realtime_detail = _build_watchlist_realtime_detail_from_candidate(
            resolved_candidate,
...
def list_watchlist_kline_annotations(tenant_slug="", stock_code="", stock_name="", details_map=None, viewer_role="dav", viewer_profile_id=""):
    ...
    if has_role_capability(normalized_role, "dav"):
...
def _normalize_watchlist_comment_row(row, detail=None, viewer_role="", viewer_profile_id=""):
    ...
    can_delete = False
    if has_role_capability(normalized_viewer_role, "dav"):
        can_delete = True
...
def list_watchlist_comments(
    ...
    if has_role_capability(normalized_viewer_role, "dav") or allow_fan_to_fan:
        return normalized_rows
    visible_rows = []
...
def delete_watchlist_comment(
    ...
    if not has_role_capability(normalized_role, "dav") and (not normalized_profile_id or normalized_profile_id != owner_id):
...
def gen_watchlist_details(allow_provider_fetch=False):
    def build_kline_series(stock_code, base_price):
        rng = random.Random(f"kline:{stock_code}")
        close = float(base_price) * (0.9 + rng.random() * 0.2)
        current_date = datetime.now() - timedelta(days=33)
    ...
    for code, seed_detail in details.items():
        ...
        realtime_detail = _build_watchlist_realtime_detail_from_candidate(
            resolved_candidate,
            stock_name=seed_detail.get("name") or code,
            allow_provider_fetch=allow_provider_fetch,
        )
        if realtime_detail:
...
def sync_news_sources(force=False):
    """Refresh the shared news-source snapshot without a model invocation."""
    ...
    # Preserve deterministic display labels without sending titles to a model
    # or overwriting separately audited historical classification records.
    annotated_items = [annotate_news_impact(item) for item in items]
    payload = dict(lake)
    payload.update({
        "cached_at": now_ts(),
        ...
        "provider": "新闻源聚合",
        "model_name": "",
        "method": "news_source_fetch_v1",
    })
    _save_json_app_setting(NEWS_LAKE_CACHE_KEY, payload)
    app.logger.info(
        "News source sync finished input_count=%d source_count=%d force=%s",
        len(annotated_items), len(payload.get("sources") or []), bool(force),
    )
    return {
        "method": "news_source_fetch_v1",
